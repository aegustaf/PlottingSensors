...
public class LightPlot extends View {
    ...
    public LightPlot(Context context) {
        super(context);
    }

    public LightPlot(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public LightPlot(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public LightPlot(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // side to side lines
        for(int i = 0; i < 7; i++) {
            canvas.drawLine(100, (this.getHeight()-100)/6*i, this.getWidth(), (this.getHeight()-100)/6*i, p);
        }
        // add y-axis measurements

        for(int i=6; i>=0; i--) {
            int num = 600 - i * 100;
            canvas.drawText(""+num, 50, (this.getHeight()-100)/6*i, p);
        }
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            x_vals.add(x_coor);
            float y_coor = (float) (this.getHeight()-100) - (((this.getHeight()-100) / 600) * array.get(i));
            float y_coor_mean = (float) (this.getHeight()-100) - (((this.getHeight()-100) / 600) * mean.get(i));
            float y_coor_sd = (float) (this.getHeight()-100) - (((this.getHeight()-100) / 600) * std_dv.get(i));
            y_vals.add(y_coor);
    ...
    public void addPoint(float num) {
        ...
        Log.v("LIGHT", "Light value is: " + num);
    }
